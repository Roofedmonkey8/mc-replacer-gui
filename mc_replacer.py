from blockstates import class_registry
...
class ReplaceCommandApp(QWidget):
    ...
    def load_blocks(self):
        try:
            with open(get_writable_block_file(), "r") as f:
                ...
                for block_id, val in raw.items():
                    if isinstance(val, str):  # short format: "mossy_cobblestone_stairs": "Stair"
                        cls = class_registry.get(val)  # ✅ use imported registry
                        self.blocks[block_id] = cls().properties() if cls else {}
                    else:
                        self.blocks[block_id] = val

            # Populate the base block dropdown
    ...
    def rebuild_state_selectors(self):
        for i, dropdown in enumerate(self.replacement_dropdowns):
            ...
            # 🧠 FIX: resolve shorthand class name (e.g. "Stair") to properties
            block_data = self.blocks.get(block_id, {})
            if isinstance(block_data, str):
                cls = self.get_block_class(block_id)
                states = cls().properties() if cls else {}
            else:
                states = block_data
...
class BlockEditorDialog(QDialog):
    ...
    def populate_table(self):
        ...
        for block_id, props in self.current_data.items():
            if isinstance(props, str):
                # Display class type only (no props)
                row = self.table.rowCount()
                self.table.insertRow(row)
                self.table.setItem(row, 0, QTableWidgetItem(block_id))
                self.table.setItem(row, 1, QTableWidgetItem("class"))
                self.table.setItem(row, 2, QTableWidgetItem(props))
    ...
    def save_changes(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if not block_id:
                continue
            if prop and prop.text().strip() == "class":
                updated[block_id] = vals.text().strip() if vals else ""
            elif prop and prop.text().strip():
                if block_id not in updated or isinstance(updated[block_id], str):
                    updated[block_id] = {}
